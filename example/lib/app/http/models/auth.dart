...
enum ProviderAuthType {
  ...
  static ProviderAuthType fromName(String? name) {
    return values.firstWhere((e) => e.name == name,
        orElse: () => throw AppExceptionConst.invalidProviderInformation);
  }
...
abstract class ProviderAuthenticated with CborSerializable, Equatable {
  ...
  T cast<T extends ProviderAuthenticated>() {
    if (this is! T) {
      throw AppException.invalidArgruments(["$T", "$runtimeType"]);
    }
    return this as T;
  }
}
  ...
  CborTagValue toCbor() {
    return CborTagValue(
        CborListValue.definite([
          CborStringValue(type.name),
          CborStringValue(key),
          CborStringValue(value)
        ]),
        type.tag);
  }
...
class DigestProviderAuthenticated extends ProviderAuthenticated {
  ...
  CborTagValue toCbor() {
    return CborTagValue(
        CborListValue.definite(
            [CborStringValue(password), CborStringValue(username)]),
        type.tag);
  }
