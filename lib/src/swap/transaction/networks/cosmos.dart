...
class SwapRouteCosmosTransactionBuilder extends SwapRouteTransactionBuilder<
  ...
  Future<void> buildTransactions({
    ...
    for (final operation in operations) {
      ...
      final transaction =
          await operation._buildTransactions(client: cosmosClient, signers: signers);
...
class SwapRouteCosmosNativeTransactionOperation
  ...
  Future<CosmosSwapFeeCoin> findFeeToken(BaseSwapCosmosClient client) async {
    final feeTokens = client.chainInfo.feeTokens;
    if (feeTokens.length == 1) {
      return feeTokens[0];
    }
    List<(CosmosSwapFeeCoin, BigInt)> balances = [];
    for (final i in feeTokens) {
      final balance = await client.getBalance(source, i.denom);
      balances.add((i, balance));
    }
    balances.sort((a, b) => b.$2.compareTo(a.$2));
    return balances[0].$1;
    // return balances
  }
  ...
  Future<Web3TransactionCosmos> _buildTransactions(
    ...
    final feeToken = await findFeeToken(client);
    final balance = await client.getBalance(source.address, tokenDenom);
    if (balance < amount.amount) {
      throw SwapConstants.insufficientAccountBalance;
    }
    final chainId = await client.chainId();
    final txRequirement = await client.getSwapTransactionRequirment(source.address, 1);
    ...
    final message = switch (isThorchainOrMaya) {
      false => cosmos_tx.MsgSend(
          ...
          amount: [Coin(denom: tokenDenom, amount: "${amount.amount}")]),
    ...
    if (fixedFee != null) {
      ...
      BigRational gasPrice = BigRational(feeToken.averageGasPrice);
      final gp = (BigRational(gasUsed) * client.chainInfo.feeMultiply);
      final feeAmount = (gp * gasPrice).ceil();
      fee = Fee(
          gasLimit: gp.toBigInt(),
          amount: [Coin(denom: feeToken.denom, amount: "${feeAmount.toBigInt()}")]);
    }
    authInfo = AuthInfo(tip: authInfo.tip, signerInfos: authInfo.signerInfos, fee: fee);
